...
### 36. Steps card content cannot scroll to the bottom (map page) — UNRESOLVED

| Field | Value |
|-------|-------|
| **Severity** | 🟠 Medium (map page steps card content clipped; other tabs scroll fine) |
| **Status** | ⚠️ **UNRESOLVED / OPEN** (2026-08-21) |
| **Symptom** | On the map page, the steps card (bottom sheet) can be pulled UP successfully, but its content cannot be scrolled DOWN to the bottom of the screen. On the pets page (and other normal-flow tabs), content scrolls to the bottom fine. User: 「一開始地圖頁面，可以拉個視窗。一開始喺中間左右，拉上去係成功嘅，但係就唔可以拉落去熒幕嘅下邊。如果我撳寵物頁面，我係可以拉到落去底部。」 |
| **Root cause (confirmed)** | The map tab container is `position:fixed; top:42; bottom:0` → it is OUT of `.scroll-wrap` flow → `scrollHeight == clientHeight` → the scroll-wrap itself has nothing to scroll on the map page. The steps card content lives inside `flex:1, overflowY:'hidden'` wrapper → when expanded content (marketplace/other-pets lists) exceeds the card height, the overflow is CLIPPED with no way to scroll. Other tabs (pets/properties/community/inventory) are normal-flow and scroll fine. Verified in browser_console: map page `wrap.maxScroll === 0` vs properties page `wrap.maxScroll === 790`. |
| **Attempted fixes (ALL REVERTED)** | ① **Approach C restructure** (map tab → normal flow, card moved below map, spacer `height:cardDragY`, card bg 0.97) — user: 「而家成個都郁唔到」 → reverted via `git checkout c30735d` + commit `48de786`. ② **overflowY:auto on card flex wrapper** (`9e444b3`) — didn't help (card content never overflows card at CARD_TARGET_H; no scrollbar appears) → reverted `341846d`. ③ **Approach D** (map tab → normal flow + `height:calc(100dvh - 104px + cardDragY)`, ONE line) — user: 「3」(not OK) → reverted `4c9fffb`. ④ **Scroll container inside card content wrapper** (`31d79e1`: `<div style={{flexShrink:0, maxHeight:'100%', overflowY:'auto'}}>` wrapping preview+extended) — verified scrollable in headless browser (`sh:168 > ch:32`, reachedBottom true) but user: 「未解決」 → left DEPLOYED (still live, harmless) but does NOT solve the user's complaint. |
| **What the user actually wants (latest clarification)** | 「拉個視窗…拉上去係成功嘅，但係就唔可以拉落去熒幕嘅下邊」 — the steps card pull-up works; scrolling the card content DOWN to the screen bottom doesn't. User compares to pets page which scrolls to bottom. **The core request: the map page should scroll like other tabs — either the whole scroll-wrap scrolls, or the card content scrolls to the bottom.** |
